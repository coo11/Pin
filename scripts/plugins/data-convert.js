...
function buttonView(title, handler, layout = _layout(1)) {
  return {
    type: "button",
    props: {
      title: title,
...
function inputView(id, layout = _layout(), placeholder = id) {
  return {
    type: "input",
    props: {
      id: id,
      ...
      borderColor: ui.color.boundary(),
      attributedPlaceholder: ui.placeholder(placeholder)
    },
    layout: layout,
    events: {
...
function rgbCopyMenu() {
  ...
  if (!color.includes(""))
    $ui.menu({
      items: [`rgb(${color.join(",")})`, color.join(",")],
      handler: title => {
        $clipboard.text = title;
        ui.toast({ text: "已复制", inset: 34 });
      },
//      finished: () => initKeyboard(false)
    });
}
...
function menuHandler(label) {
  ...
  $ui.menu({
    ...
    handler: (title, index) => {
      ...
      type = index;
      $cache.set("converType", index);
    },
    finished: cancelled => {
      if (cancelled) rotate(0);
      else {
        $("body").views[0].remove();
        $("body").add(convertView[type]);
        label.text = convertTitle[type];
        initKeyboard(false);
      }
    }
  });
