const bakStamp = $context.query.bak;
if (bakStamp) {
  const src = `shared://${bakStamp}-items.json`;
  const success = $file.move({ src, dst: "assets/text-items.json" });
  await $wait(1);
  success && $file.delete(src);
}

const m = ["app", "today", "keyboard"];
const n = $app.env.toString().slice(0, 1) - 1;
...
$cache.get("theme") === undefined && $cache.set("theme", ["systemLink"])
...
const module = require(`scripts/${m[n]}`);
